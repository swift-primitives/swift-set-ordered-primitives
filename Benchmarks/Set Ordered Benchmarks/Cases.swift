...
typealias MoveOrdered<E: Hash.Key & ~Copyable> = Set<OrderedColumn<E>>.Ordered

typealias CoWOrdered<E: Hash.Key & SendableMetatype> = Set<Ownership.Shared<E, OrderedColumn<E>>>.Ordered
...
extension Bench {
    ...
    static func setOrderedCases() -> [Result] {
        var results: [Result] = []
        ...
        for n in sizes {
            ...
            results.append(Result(
                ...
                perOpNs: sample(opsPerBatch: buildOps) {
                    var acc = 0
                    for _ in 0..<reps {
                        var s = MoveOrdered<Int>(minimumCapacity: .zero)
            ...
            results.append(Result(
                ...
                perOpNs: sample(opsPerBatch: buildOps) {
                    var acc = 0
                    for _ in 0..<reps {
                        var s = CoWOrdered<Int>(minimumCapacity: .zero)
                        for i in 0..<n { _ = s.insert(i &+ seed) }
                        acc &+= s.first ?? 0
                    }
                    sink(acc)
            ...
            var s = MoveOrdered<Int>(minimumCapacity: count(n))
            for i in 0..<n { _ = s.insert(i) }
            var c = CoWOrdered<Int>(minimumCapacity: count(n))
        ...
        // The evict curves: rolling key window keeps occupancy at n; keys
        // advance monotonically so every pair removes a real element and
        // inserts a genuinely fresh one. One op = one remove+insert pair.
        for n in curveSizes {
