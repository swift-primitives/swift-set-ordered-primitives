...
extension Set.Ordered where Element: ~Copyable {
    ...
    ///
    /// Composes `Buffer<Storage<Element>.Contiguous<Memory.Heap<Element>>>.Linear.Bounded` for element storage and
    ...
    public struct Fixed {
        ...
        @usableFromInline
        internal var buffer: Buffer<Storage<Element>.Contiguous<Memory.Heap<Element>>>.Linear.Bounded
        ...
        public init(capacity: Index_Primitives.Index<Element>.Count) throws(__SetOrderedFixedError<Element>) {
            self.buffer = Buffer<Storage<Element>.Contiguous<Memory.Heap<Element>>>.Linear.Bounded(minimumCapacity: capacity)
