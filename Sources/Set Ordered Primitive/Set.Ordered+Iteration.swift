...
//
// `consume()` is the one ops-bound member (its `Sequence.Consume.View` return
// type pulls `Sequence_Primitives`, kept out of this lean type module), so it
// reaches storage through the single `package` accessor below — named, not
// underscored, and non-`@usableFromInline` since the cold `consume()` is
// non-`@inlinable`.
...
extension Set.Ordered where Element: Copyable {
    ...
    /// conformance (`consume()`).
    package consuming func takeBuffer() -> Buffer<Element>.Linear {
        makeUnique()
